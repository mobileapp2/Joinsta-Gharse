...
public class EditAddressActivity extends AppCompatActivity {
    ...
    private void setDefault() {
        addressDetails = (AddressModel.ResultBean) getIntent().getSerializableExtra("addressDetails");

        latitude = addressDetails.getLatitude();
        longitude = addressDetails.getLangtitude();
        edtAddress.setText(addressDetails.getAddress_line1());
        edtCountry.setText(addressDetails.getCountry());
        edtState.setText(addressDetails.getState());
    ...
    private void submitData() {
        ...
        if (Utilities.isNetworkAvailable(context))
            new UpdateAddress().execute(mainObj.toString().replace("\'", Matcher.quoteReplacement("\\\'")));
        else
